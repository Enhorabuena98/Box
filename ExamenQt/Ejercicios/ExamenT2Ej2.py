...
#2 menu
    def crear_menu(self):
        ...
        menu = barra_menu.addMenu("&Asignaturas")
#3 rutas
    
        ruta1 = os.path.join(os.path.dirname(__file__),"../ex1.png")
        ruta2 = os.path.join(os.path.dirname(__file__),"../ex2.png")
#4 accion1
        accion1 = QAction(QIcon(ruta1),"&Prioridad de asignaturas\n2ºDAM",self)
        accion1.setWhatsThis("Nos indica cual es la asignatura mas importante de \n2ºDAM")
        accion1.setShortcut("Ctrl+P")
        accion1.triggered.connect(self.imprimir_dock)
#5 accion2
        accion2 = QAction(QIcon(ruta2),"&Ayuda",self)
        accion2.setWhatsThis("Activamos el modo ayuda")
        accion2.setShortcut("shift+F1")
...
#9 dock y textedit
    def crear_dock(self):
        ...
        self.dock = QDockWidget("El subsconciente del alumno", self)
...
# def2
    def imprimir_dock(self):
        self.dock.widget().append("La asignatura mas importante de 2ºDAM es Desarroll de interfaces")
# main
